...
export class VentaComponent implements OnInit {
  ...
  listaProductos: Producto[] = []; // Lista de productos que tenemos almacenados en la base de datos
  listaProductosFiltro: Producto[] = []; // Autocompletado de productos

  listaProductosParaVenta: DetalleVenta[] = []; // Lista de productos que se van a vender
  bloquearBotonRegistrar: boolean = false;
  
  productoSeleccionado!: Producto; // Se guardar temporalmente para pasar a listaProductos
      ...
      this._ventaService.registrar(request).subscribe({
        next: (response) => {
          if(response.status){
            ...
            this._utilidadService.mostrarMensaje("No se pudo regitrar la venta", "Oops!");
          }
        },
        complete: () => {
